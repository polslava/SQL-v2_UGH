  ...
  CREATE OR REPLACE FORCE VIEW "UGH"."V_PLANFACT_HOURSUM_HALF_SHOV" ("SHIFTHOUR_HALF_T", "SHIFTHOUR_HALF", "SHIFTHOUR", "PLAN_HOUR_HALF", "SHOVID", "VOLUME", "PLANFACT_PERCENT", "PLANFACT_PERCENT100", "PLANFACT") AS 
  SELECT

    sel2.shifthour_half_t, sel2.shifthour_half, sel2.shifthour, sel2.plan_hour_half, sel2.shovid
    -- Богатырев Е.А. 17.11.2023
    -- Это условие не канает - sel2.volume>0
    -- Если экскаватор встал на ремонт и у него нули в факте, то с таким условием его минуса станут нулями.
    -- Цель условия ясна - сделать нолики для будущих периодов. Оно немного другое - нулим там, где нет плана и
    -- получасья таблцы больше текщего получасья смены.
    -- Для экскваторов, у которых нет текущего факта добавлено преобразование nvl()
  /*
    ...
    , (case when sel2.volume > 0 then round(sel2.volume-sel2.plan_hour_half,2) else 0 end) as planfact
    */

    , (case when (plan_hour_half > 0 and sel2.shifthour_half<=UGH.Getcurhalfhour()) then nvl(sel2.volume,0) else 0 end) as volume
    , (case when (plan_hour_half > 0 and sel2.shifthour_half<=UGH.Getcurhalfhour()) then round(nvl(sel2.volume,0)/(case when sel2.plan_hour_half=0 then sel2.volume else sel2.plan_hour_half end),2) else 0 end) as planfact_percent
    , (case when (plan_hour_half > 0 and sel2.shifthour_half<=UGH.Getcurhalfhour()) then round(nvl(sel2.volume,0)/(case when sel2.plan_hour_half=0 then sel2.volume else sel2.plan_hour_half end),2)*100 else 0 end) as planfact_percent100
    , (case when (plan_hour_half > 0 and sel2.shifthour_half<=UGH.Getcurhalfhour()) then round(nvl(sel2.volume,0)-sel2.plan_hour_half,2) else 0 end) as planfact
...
/*(select '01_1' as shifthour_half_t, 1  as shifthour_half, 1 as shifthour
    , round((s.plan_volume/2)/10.5,1) as plan_hour_half, s.shovid
from v_shovels_in_shift_planfact s
union
select '01_2' as shifthour_half_t, 2  as shifthour_half, 1 as shifthour
    , round((s.plan_volume/2)/10.5,1) as plan_hour, s.shovid
from v_shovels_in_shift_planfact s
union
